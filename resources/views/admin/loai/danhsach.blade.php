            ...
            <tbody>
                @foreach($loai as $value)
                    <tr>
                        ...
                        <td class="text-center"><a href="{{ route('admin.loai.sua', ['id' => $value->id]) }}"><i class="fa fa-edit"></i></a></td>
                        <td class="text-center"><a href="{{ route('admin.loai.xoa', ['id' => $value->id]) }}" onclick="return confirm('Bạn có muốn xóa loại {{ $value->tenloai}} không?')"><i class="fa fa-trash-alt text-danger"></i></a></td>
                    </tr>
                @endforeach
            </tbody>
        </table>
